...
(function () {
  ...
  var formFields = ''
    + '<div class="pb-row">'
    +   '<div><label class="pb-label" for="pb-fn">Athlete First Name</label><input class="pb-input" type="text" id="pb-fn" name="first_name" placeholder="e.g. Jamie" required/></div>'
    +   '<div><label class="pb-label" for="pb-ln">Athlete Last Name</label><input class="pb-input" type="text" id="pb-ln" name="last_name" placeholder="e.g. Smith" required/></div>'
    + '</div>'
    + '<div class="pb-row">'
    +   '<div><label class="pb-label" for="pb-age">Athlete Age</label><input class="pb-input" type="number" id="pb-age" name="age" placeholder="e.g. 9" min="3" max="18" required/></div>'
    +   '<div><label class="pb-label" for="pb-gender">Gender</label><select class="pb-select" id="pb-gender" name="gender" required><option value="">Select…</option><option>Male</option><option>Female</option><option>Prefer not to say</option></select></div>'
    + '</div>'
    + '<div><label class="pb-label" for="pb-email">Parent / Guardian Email</label><input class="pb-input" type="email" id="pb-email" name="email" placeholder="parent@example.com" required/></div>'
    + '<div><label class="pb-label" for="pb-phone">Parent / Guardian Phone</label><input class="pb-input" type="tel" id="pb-phone" name="phone" placeholder="04XX XXX XXX" required/></div>'
    + '<div><label class="pb-label" for="pb-program">Interested Program</label><select class="pb-select" id="pb-program" name="program" required><option value="">Select a program…</option><option>Miniball (Ages 5–10)</option><option>Academy (Ages 5–15)</option><option>Teams / Competition (Ages 5–15)</option><option>Not sure — advise me</option></select></div>'
    + '<div><label class="pb-label" for="pb-loc">Preferred Location</label><select class="pb-select" id="pb-loc" name="location"><option value="">Select a venue…</option><optgroup label="Miniball"><option>Leichhardt (Sydney Secondary College) · Sat</option><option>Green Square (Green Square Public School) · Sun</option></optgroup><optgroup label="Academy"><option>Maroubra (Heffron Centre)</option><option>Surry Hills (Inner Sydney High School)</option><option>Zetland (Green Square Public School)</option><option>Leichhardt (Sydney Secondary College)</option><option>Marrickville (Wilkins Public School)</option><option>Maroubra (South Sydney High School)</option><option>Eastern Suburbs (Centennial Parklands)</option></optgroup><optgroup label="Teams"><option>Alexandria</option></optgroup></select></div>'
